  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      if (u) {
        ...
        setUserData(data);
        setBoosts({
          freezeTime: data?.boosts?.freezeTime || 0,
          doubleXP: data?.boosts?.doubleXP || 0,
          fiftyFifty: data?.boosts?.fiftyFifty || 0,
          hint: data?.boosts?.hint || 0,
          skip: data?.boosts?.skip || 0,
          golden: data?.boosts?.golden || 0,
          vip: data?.boosts?.vip || false,
        });
      } else {
        setUserData(null);
      }
